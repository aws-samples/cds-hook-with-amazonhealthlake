...
async function getObservationResources(patientId) {
    console.log('Entering new getObservationResources');
    ...
    var request = new HttpRequest({
        ...
        hostname: domain,
        method: 'GET',
        path: '/datastore/' + HL_DATASTORE_ID + '/r4/Observation',
...
exports.runCQLQuery = async(event, context) => {
    ...
    const observationResource = await getObservationResources(patientId);
    console.log('FHIR Observation Resource received ' + observationResource);
    ...
    patientConditionBundle['entry'].push(JSON.parse(observationResource)['entry'][0]);
